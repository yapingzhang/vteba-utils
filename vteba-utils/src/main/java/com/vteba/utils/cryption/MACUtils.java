...
/**
 * 消息认证码算法，mac（Message Authentication Code，消息认证码算法）是含有密钥散列函数算法，
 * 兼容了MD和SHA算法的特性，并在此基础上加上了密钥。
 * <p>
 * 1、双方约定算法和密钥，双方拥有相同的密钥。
 * 2、A使用密钥对消息进行散列后，将明文和散列消息一起发送给B。
 * 3、B收到消息后，使用相同的密钥和算法，对消息进行散列，将其和B发送来的消息进行比较
 * 
 * @author yinlei
 * @date 2012-12-2
 */
